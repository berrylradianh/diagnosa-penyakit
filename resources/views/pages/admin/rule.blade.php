      ...
      <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
        <div class="widget-content widget-content-area br-6">
          <table id="zero-config" class="table dt-table-hover" style="width:100%">
            ...
            <tbody>
              @foreach($percobaans as $percobaan)
              <tr class="text-center">
                <td>{{ $percobaan->kode }}</td>
                @foreach($gejalas as $gejala)
                <td>{{ $percobaan->has_gejala($gejala->kode) ? '1' : '0' }}</td>
                @endforeach
                <td>{{ $percobaan->penyakit?->nama }}</td>
                <td class="text-center">
                  ...
                  <a href="{{ url('rule-admin/'.$percobaan->id . '/edit') }}"><button class="mt-2 btn btn-secondary btn-sm">Edit</button></a>
                </td>
              </tr>
              @endforeach
            </tbody>
